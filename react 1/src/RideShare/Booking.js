...
function Booking(prop){
    ...
    function getAmount(){
        axios.get("http://localhost:8083/findbyregNumber?regNumber="+prop.vehicleId).then((res)=>{
            setVeh(res.data);
            console.log("price  Vejvile"+veh.pricePerKm);
        })
    }
    useEffect(getAmount,[]);
    ...
    function confirmBooking(){
        ...
        axios.get("http://localhost:8084/owneraddConfirmations?ownerconfirmationId="+prop.bookingId+"&pickUpLocation="+prop.pick+"&droppingLocation="+prop.drop+"&kms="+parseInt(prop.kms)+"&ownerconfirmationDate="+today+"&regNumber="+prop.vehicleId+"&userName="+prop.userName+"&userEmail="+prop.userEmail).then((res)=>{
            

            axios.get("http://localhost:8084/deleteConfoBooking?id="+prop.bookingId); 
            axios.get("http://localhost:8085/addOwnerMessage?id="+prop.bookingId+"&msgedDate="+today+"&adminMsg="+msg+"&userEmail="+prop.userEmail);
    ...
    function deleteBooking(){
        ...
        alert("into delete")
        axios.get("http://localhost:8084/deleteConfoBooking?id="+prop.bookingId).then((res)=>{
